Shader "Lux/Bumped Vertex Colored Specular Debug" {

Properties {
	_Color ("Diffuse Color(RGB) Metal(-A)", Color) = (1,1,1,1)
	_MainTex ("Base (RGB) Alpha (A)", 2D) = "white" {}
	_ShapeAdj ("Specular Roughness (R) Ambient Occlusion (B)", Color) = (1,1,1,1)
...
//	#define LUX_AO_OFF
	...
	void surf (Input IN, inout SurfaceOutputLux o) {
		...
		// Diffuse Albedo
		o.Albedo = diff_albedo.rgb * _Color.rgb * _Color.a;
		o.Alpha = diff_albedo.a;
		o.Normal = UnpackNormal(tex2D(_BumpMap, IN.uv2_BumpMap));
		// Specular Color
		o.SpecularColor = (diff_albedo.rgb * _Color.rgb * (1.0 - _Color.a)) + (0.3 * _Color.a);
		// Roughness – gamma for BlinnPhong / linear for CookTorrence
		o.Specular = LuxAdjustSpecular(spec_albedo.r * _ShapeAdj.r);
