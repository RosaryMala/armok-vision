...
Shader "Skybox/Complex"
{
    Properties
    {
        _StarTint("Star Tint", Color) = (.5, .5, .5, .5)
        [NoScaleOffset] _StarMap("Starmap   (HDR)", Cube) = "grey" {}
        [Gamma] _StarExposure("Star Exposure", Range(0, 8)) = 1.0
        _StarRotation("Star Rotation", Vector) = (0,0,0,0)
        [NoScaleOffset] _MoonDiffuse("Moon Diffuse (RGBA)", 2D) = "white" {}
        [NoScaleOffset] _MoonBump("Moon Normal Map", 2D) = "bump" {}
        [Gamma] _MoonAmbient("Moon Ambient", Range(0, 1)) = 0.1
        [Gamma] _MoonExposure("Moon Exposure", Range(0, 8)) = 1.0
        _MoonSize("Moon Size", Range(0,1)) = 0.04
        _MoonRotation("Moon Rotation", Vector) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass 
        {
            ...
            sampler2D _MoonDiffuse;
            sampler2D _MoonBump;
            float4x4 _MoonRotationMatrix;
            half _MoonExposure;
            half _MoonAmbient;
            ...
            struct v2f {
                ...
                #endif
                float3 starCoords : TEXCOORD4;
                float3 moonCoords : TEXCOORD5;
            ...
            v2f vert(appdata_t v)
            {
                ...
                OUT.starCoords = mul(v.vertex.xyz, _StarRotationMatrix).xyz;
                OUT.moonCoords = mul(v.vertex.xyz, _MoonRotationMatrix).xyz;
                return OUT;
            }
            ...
            {
                half3 atmoCol = half3(0.0, 0.0, 0.0);

                half4 starTex = texCUBE(_StarMap, IN.starCoords);
                half3 starColor = DecodeHDR(starTex, _StarMap_HDR);
                starColor = starColor * _StarTint.rgb * unity_ColorSpaceDouble.rgb;
                starColor *= _StarExposure;
                
                half2 moonCoords = IN.moonCoords.xy;
                moonCoords /= _MoonSize;
                half4 moonColor = tex2D(_MoonDiffuse, moonCoords + float2(0.5, 0.5));
                moonColor.rgb *= _MoonExposure;
                moonColor *= step(moonCoords.x*moonCoords.x + moonCoords.y*moonCoords.y, 0.25);
                moonColor *= step(IN.moonCoords.z, 0);

                half3 moonNormal = UnpackNormal(tex2D(_MoonBump, moonCoords + float2(0.5, 0.5)));
                half3 moonLight = _WorldSpaceLightPos0;
                moonNormal = mul(moonNormal, transpose(_MoonRotationMatrix));
                //moonNormal *= half3(1, -1, 1);

                moonColor.rgb *= clamp(pow(dot(moonLight, moonNormal), 0.5), _MoonAmbient, 1);

                starColor = lerp(starColor, moonColor, moonColor.a);
                ...
                atmoCol = lerp(IN.skyColor, IN.groundColor, saturate(y));

                #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
                    if (y < 0.0)
                    {
                        atmoCol += IN.sunColor * calcSunAttenuation(_WorldSpaceLightPos0.xyz, -ray) * (1 - moonColor.a);
                    }
                #endif

                #if defined(UNITY_COLORSPACE_GAMMA) && !SKYBOX_COLOR_IN_TARGET_COLOR_SPACE
                    atmoCol = LINEAR_2_OUTPUT(atmoCol);
                #endif

                atmoCol += starColor;

                return half4(atmoCol, 1);
            }
            ENDCG
        }
    }
    CustomEditor "ComplexSkyEditor"
    Fallback Off
}
